describe('When logging using error method', function() {
    ...
    it('Should log expected caption & description',function () {
        common.executeTest($scope.error('error logging','testing logging using error method'),
                    function(requestBody) {
                        var data = JSON.parse(requestBody).logMessages[0];
                        
                        expect(data.category).toEqual('test');
                        expect(data.caption).toEqual('error logging');
                        expect(data.description).toEqual('testing logging using error method');
    ...
    it('Should log expected exception', function () {
        ...
        common.executeTest($scope.error('error logging','testing logging using error method',suppliedException),
                    function(requestBody) {
                        var data = JSON.parse(requestBody).logMessages[0];
    ...
    it('Should log expected details', function (){
        common.executeTest($scope.error('error logging','testing logging using error method',null,"<data>details</data>"),
                    function(requestBody) {
                        var data = JSON.parse(requestBody).logMessages[0];
